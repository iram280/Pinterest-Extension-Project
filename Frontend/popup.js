document.addEventListener("DOMContentLoaded", () => {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    // Checks for the current open tab
    const activeTab = tabs[0]; // Stores the current open tab
    const url = activeTab.url; // Stores URL of the current open tab

    const pinterestBoardRegex =
      /^https:\/\/www\.pinterest\.com\/[^/]+\/[^/]+\/$/;

    // Checks for valid Pinterest board url
    if (pinterestBoardRegex.test(url)) {
      console.log("Valid Pinterest board URL:", url);
      initializeExtension(); // Initializes extension for use
    } else {
      alert("This extension only works with Pinterest boards"); // Alerts user to requirements
      chrome.tabs.update(activeTab.id, { url: "https://www.pinterest.com/" }); // Redirects user to Pinterest home page
      document.body.innerHTML = "<h2>Navigate to a Pinterest board!</h2>"; // Fills extension body with next step
    }
  });

  // Main function that runs extension logic
  function initializeExtension() {
    const clothes = document.querySelectorAll(".clothes"); // Clothes class selection
    const upButton = document.querySelector(
      ".result__filter-btns button:first-child"
    ); // Up button selection
    const downButton = document.querySelector(
      ".result__filter-btns button:last-child"
    ...
    function checkFilter() {
      ...
      if (selectedCategory === "Men") {
        switch (categoryFilter) {
          case "pants":
            categoryFilter = "pantsMen"; // Sets filter to men's pants
            break;
          case "shoes":
            categoryFilter = "shoesMen"; // Sets filter to men's shoes
            break;
          default:
            categoryFilter = "Men"; // Default filter to men's shirts
        }
      } else {
        switch (categoryFilter) {
          case "pants":
            categoryFilter = "pantsWomen"; // Sets filter to women's pants
            break;
          case "shoes":
            categoryFilter = "shoesWomen"; // Sets filter to women's shoes
            break;
          default:
            categoryFilter = "Women"; // Default filter to women's shirts
        }
      }
    ...
    function updateCategoryTitle(category) {
      const categoryButton = document.querySelector(
        ".header__section-dropdown button"
      );
      categoryButton.textContent = `${category} `;
      categoryButton.appendChild(document.createElement("i")).className =
    ...
    const fetchProducts = async (keyword, category) => {
      try {
        const response = await fetch(
          `https://pinsift-server.onrender.com/api/products?keyword=${keyword}&category=${category}`
        ); // Calls eBay API through proxy server
    ...
    // Fetch products by calling the Flask API
    const fetchResults = async (boardUrl) => {
      try {
        ...
        const response = await fetch("http://127.0.0.1:5000/process-image", {
          ...
          body: JSON.stringify({
            url: boardUrl, // Send the board URL to the Flask backend
          }),
        });
        ...
        if (response.ok) {
          // Handle the returned analysis results
          displayProducts(data.analysis_results);
        } else {
    ...
    const displayProducts = (data) => {
      ...
      const items =
        data.findItemsAdvancedResponse[0]?.searchResult[0]?.item || []; // Sets the items returned from the eBay APi to specific list of items
      ...
      items.forEach((item) => {
        ...
        let imageUrl =
          item.prictureURLSuperSize ||
          item.pictureURLLarge ||
          item.galleryURL[0] ||
          "images/placeholder.png"; // Retrives the best quality product image available from the options the eBay API gives
        ...
        const price =
          ...
          "No Price Available"; // Sets the product price
        const condition =
          item.condition?.[0]?.conditionDisplayName[0] ||
          "No Condition Available"; // Sets the product condition
