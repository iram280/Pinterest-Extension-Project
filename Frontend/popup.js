document.addEventListener("DOMContentLoaded", () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {    // Checks for the current open tab
        const activeTab = tabs[0];  // Stores the current open tab
        const url = activeTab.url;  // Stores URL of the current open tab

        const pinterestBoardRegex = /^https:\/\/www\.pinterest\.com\/[^/]+\/[^/]+\/$/;  // Regular expression for Pinterest Boards
